...
- [x] Site scaffold built and populated with real pilot data (Astro, static) — 2026-07-27, **not deployed publicly** (deployment is now a standalone decision; the HRL-002 governance gate cleared 2026-07-27)
- [x] Gold-standard human audit round 1 (methodology.md §8) — full 31-incident review completed 2026-07-27, 0 content errors; second round with multiple raters still needed for a publishable precision number
- [x] Pre-release governance: data license decided (CC BY-SA 4.0, HRL-005), privacy QA signed off (HRL-008), family risk assessment signed off (HRL-002) — all 2026-07-27
- [x] Public submission intake — web form (`/submit/` + one serverless function) and GitHub issue template, both feeding the same pre-screen + human-review flow (HRL-021/022); Vercel deployment pending account setup (`site/DEPLOY.md`)
- [x] `ahid-cn-dataset-v0.1` **published on Zenodo** — DOI [10.5281/zenodo.21462312](https://doi.org/10.5281/zenodo.21462312), released 2026-07-20, nearly three months ahead of the Oct 2026 target
- [ ] Methods white paper (SocArXiv) — Nov–Dec 2026 target
- [ ] Six-locale site content (currently zh-Hans only; gated on HRL-010) and public deployment decision — after interview season
...
pipeline/
  ...
  populate_responses.py, export_site_data.py, export_dataset_csvs.py
                                  # Stages 0-5.5; database and archived snapshots are gitignored
site/
  src/data/incidents.json        # Generated public export (see site/README.md); site itself unpublished
release/
  ahid-cn-dataset-v0.1/          # Local staging build of the versioned dataset package
                                  # (data/ CSVs + documentation/); not yet released, see its own README.md
```
...
**Dataset**: Deng, Nanyi (2026). *AHID-CN Dataset v0.1* [Data set]. Zenodo. https://doi.org/10.5281/zenodo.21462312

**Code/repository**: see `CITATION.cff`.
