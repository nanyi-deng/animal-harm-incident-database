...
**去重：保留而非合并。** CAIL2018 把同一份判决分散在训练、测试、验证各分片，因此系统对事实文本做相似度比对、标出近重复对，却刻意保留双方而不自动合并。理由是同一真实案件的不同副本可以各自携带不同的证据元数据（例如一份记录了"盗窃后追回"、另一份只记录了"疑似偷狗"），合并会抹掉这层区分。代价是名义行数大于有效唯一案件数，所以任何计数分析都必须先去重——当前版本约为 410 个唯一案件对应 492 个名义行，`pipeline/census_validate.py` 会算出并报告这个差额，提醒使用者不要把行数当案件数。
